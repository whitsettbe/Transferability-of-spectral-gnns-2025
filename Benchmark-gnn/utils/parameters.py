...
def param_weights_and_biases(in_shape, out_shape, bias=True):
    """
    ...
    """
    weight = torch.nn.Parameter(torch.empty(in_shape + out_shape))
    torch.nn.init.kaiming_uniform_(weight, a=5**.5)
    if bias:
        fan_in = torch.prod(torch.tensor(in_shape)).item()
        bound = 1 / fan_in**.5
        bias = torch.nn.Parameter(torch.empty(out_shape))
